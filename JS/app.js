...
class Wizard {
	...
	clickFeed() {
		...
		if (currentWizard.age >= 100) {
			...
			window.alert(
				"Your skills need more work. The Wizard Sanctum's doors remain locked at this time. Try again later."
			);
		}
	...
	clickTrain() {
		if (currentWizard.skill >= 10) {
			...
			if (currentWizard.level > 9) {
				window.alert(
					'Wow, you made it to the status of Archmage! A real Archmage!'
				);
				winGame();
			}
			currentWizard.level++;
...
function getInput() {
	...
	skillClass = window.prompt(
		'Do you choose fire ball, lightning strike, or ice blast?'
	);
...
function winGame() {
	clearInterval(hungerInterval);
	window.alert(
		'The Wizard Sanctum approaches, and offers you a position amongst the scribes. GG'
	);
}
...
function wizardEvolve() {
	wizardForm = [
		new Wizard(newName, favoriteFood, 0, 0),
		new MatureWizard(newName, favoriteFood, age, skill, fatigue, hunger, level),
		new GrandWizard(newName, favoriteFood, age, skill, fatigue, hunger, level),
		new Archmage(newName, favoriteFood, age, skill, fatigue, hunger, level),
	];
	...
	if (k === 1) {
		$('.display').attr(
			'src',
			'https://cdn.pixabay.com/photo/2019/11/05/12/02/lego-4603354_960_720.jpg'
		);
	} else if (k === 3) {
		$('.display').attr(
			'src',
